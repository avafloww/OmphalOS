...
impl BoardData {
    ...
    pub fn init(self) -> Vec<impl Driver> {
        ...
        // needs to be pulled high, otherwise we get no peripheral power (i.e. no display)
        ...
        let spi2_bus = SPI2_BUS.init(Mutex::new(RefCell::new(
            Spi::new(self.spi2, 60u32.MHz(), SpiMode::Mode0, &self.clocks)
                .with_sck(self.io.pins.gpio18)
                .with_mosi(self.io.pins.gpio13),
        )));

        // SPI chip select pins
        // set them all to high to start (disabled)
        let display_cs = Output::new(self.io.pins.gpio12, Level::High);
        let _lora_cs = Output::new(self.io.pins.gpio5, Level::High);

        // initialize display
        let delay = Delay::new(&self.clocks);
        let enable_pin = Output::new(self.io.pins.gpio45, Level::High);
        let dc_pin = Output::new(self.io.pins.gpio38, Level::Low);
